...
function freshHome(): string {
  const dir = join(
    tmpdir(),
    `bap-cli-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
...
describe("CLI: create", () => {
  ...
  test("creates identity with --name", () => {
    ...
    const config = JSON.parse(
      readFileSync(join(testConfigDir, "identity.json"), "utf-8")
    );
...
describe("CLI: export / import roundtrip", () => {
  test("export and import preserves identities", () => {
    ...
    const importResult = Bun.spawnSync(
      ["bun", "src/cli.ts", "import", backupFile],
      {
        cwd: "/Users/satchmo/code/bap",
...
describe("CLI: version and help", () => {
  test("--version matches package.json", () => {
    // Read the expected version from package.json so a stale hardcoded
    // CLI version fails here instead of shipping (it sat at 0.2.0
    // through the 0.3.x releases).
